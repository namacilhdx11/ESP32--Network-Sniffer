...
#define STICK_TO_ONE_CHANNEL 2 // Type in the Channel of your Choice. if you want to hop channels in regular time intervals, type something wich is not a channel X > 13 for example
...
void wifi_sniffer_packet_handler(void* buff, wifi_promiscuous_pkt_type_t type){ //This is the Call when ever a Package goes by.
  ...
  String ppkgSSID = "";
  uint16_t ppktStatus = 0;
  uint16_t ppktReason = 0;
  ...
  Serial.println();
  if(PACKAGE_INSIGHT_MODE){ //Do the Actuall interpreting part
    ...
    case 0:
      ...
      for(int i = 0; i < 6;i++){
        Serial.print(HexToString(hdr->addr3[i]) + " "); 
      }            
    break;
    case 1:
      Serial.print("Recieving/Destination(RA/DA) Adress : ");
    ...
    case 3:
      ...
      Serial.print("Source Adress (SA)                    : ");
      for(int i = 0; i < 6;i++){
        Serial.print(HexToString(hdr->addr4[i]) + " "); 
      }            
      break;
    }
    Serial.println();  
    //uint16_t BeaconInterval = ((ppkt->payload[55] << 8) | ppkt->payload[56]); 
    //Serial.println("Interval: " + String(BeaconInterval)); 
    
    
    if(PPktType == "Association Request"){
      
      ppktStatus = (ppkt->payload[25]<<8) | ppkt->payload[24];
      ...
      ppktReason = (ppkt->payload[27]<<8) | ppkt->payload[26];
